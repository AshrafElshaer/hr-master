...
alter table
  users
add
  column organization_id uuid references organizations (id) on delete
set
  null;

-- Create a function to update the organization_id column in the users table
CREATE FUNCTION on_organization_insert () RETURNS TRIGGER AS $ $ BEGIN
UPDATE
  public.users
SET
...
CREATE FUNCTION current_user_organization_id() RETURNS UUID AS $ $ BEGIN RETURN (
  SELECT
    organization_id
  FROM
    public.users
...
AFTER
INSERT
  ON public.organizations FOR EACH ROW EXECUTE FUNCTION on_organization_insert ();
...
select
  using (
    (
      SELECT
        (auth.uid() = organizations.owner_id)
    )
  );

create policy organization_insert_policy on organizations for
insert
  with check (
    (
      (
...
create policy organization_update_policy on organizations for
update
  using (
    auth.uid () = owner_id
    or (
