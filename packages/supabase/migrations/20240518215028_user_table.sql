...
CREATE FUNCTION current_user_role() RETURNS text AS $ $ DECLARE role text;
...
CREATE FUNCTION current_user_department_id () RETURNS UUID AS $ $ DECLARE current_user_department_id UUID;

BEGIN
SELECT
  department_id INTO current_user_department_id
FROM
  public.users
WHERE
  id = auth.uid();
...
CREATE FUNCTION current_user_organization_id () RETURNS UUID AS $ $ DECLARE current_user_organization_id UUID;

BEGIN
SELECT
  organization_id INTO current_user_organization_id
FROM
  public.users
WHERE
  id = auth.uid();
...
create policy "Users or owner or manager can insert their own profile." on users for
insert
  with check (
    (
      select
...
create policy "Users or owner or manager can update own profile." on users for
update
  using (
    auth.uid () = id
    or current_user_role() = 'owner'
    and current_user_organization_id() = current_user_organization_id()
    or current_user_role() = 'manager'
    and current_user_organization_id() = current_user_organization_id()
  );
...
insert into
  public.users (id, email, avatar_url)
values
  (
    new.id,
    new.email,
